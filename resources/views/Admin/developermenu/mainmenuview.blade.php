                      ...
                      @endphp
                        ...
                        @endphp
                         <tr class="gradeX" id="tr-{{$st->id}}">
                            ...
                            <td>{{$st->Link_Name}}</td>
                            <td>{{$st->routeName}}</td>
                            <td><center><i class="{{$st->icon}}"></i></center></td>
                            <td>
                              @if($st->status=="1")
...
<script  type="text/javascript">
  function Edit() {
      ...
      $.ajax({
        ...
        url: "{{URL::to('main-menu-edit')}}",
        type: 'POST',
        data: data,
        success:function(data) {
          if(data=="null"){
            $('#confirm_msg').html('<b class="alert alert-danger" style="padding:0px 10px 0px 10px">Please Select A MainMenu To Edit</b>');
          }else if(data=="max"){
            $('#confirm_msg').html('<b class="alert alert-danger" style="padding:0px 10px 0px 10px">Please Select Maximum Of One MainMenu To Edit</b>');
          }else{
            window.open("{{URL::to('main-menu')}}/"+data+"/edit","_parent"); 
          }
        }
      });
}
      ...
      $.ajax({
        ...
        success:function(data) {
          if(data=="1"){
            ...
            $('#confirm_msg').html('<b class="alert alert-danger" style="padding:0px 10px 0px 10px">Please Select One or More MainMenu To Delete</b>');
          }
        }
      });
  }else{
