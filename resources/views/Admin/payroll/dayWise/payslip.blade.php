...
<div class="container">
  <table class="table table-striped table-bordered">
    ...
    <tr>
      <th>Duration </td>
      <th>{{$payroll_date_from}} to {{$payroll_date_to}}</th>
      <th>Salary Execuation Date From</td>
      <th>{{$PayrollSummery->date_of_execution}}</th>
    </tr>
    <tr>
      <td colspan = "2" valign="top" style="padding:0px;width: 50%">
        <table class="table table-striped" style="border:0px">
          ...
          <tbody>
          @if(isset($payroll[0]))
          @foreach($payroll as $head)
            @if($head->head->head_type=="1" && $head->amount>0)
              <tr>
                <td>{{$head->head->head_name}}</td>
                <td align="right">{{Controller::decimal((($head->amount*12.17)/365)*$salaryDays)}} BDT</td>
              </tr>
            @endif
          @endforeach
          @endif
      ...
      <td colspan = "2" valign="top" style="padding:0px;width: 50%">
        <table class="table table-striped">
          ...
          <tbody>
            @if(isset($payroll[0]))
            @foreach($payroll as $head)
              @if($head->head->head_type=="0" && $head->amount>0)
                <tr>
                  <td>{{$head->head->head_name}}</td>
                  <td align="right">{{Controller::decimal((($head->amount*12.17)/365)*$salaryDays)}} BDT</td>
                </tr>
    ...
    <tr>
      <td colspan="2" style="padding:0px;">
        <table class="table">
          <thead>
            <tr>
              <td class="strong-text">Earning </td>
              <td class="text-right strong-text">{{$PayrollSummery->addition}} BDT</td>
            </tr>
      ...
      <td colspan="2" style="padding:0px;">
        <table class="table">
          <thead>
            <tr>
              <td class="strong-text">Deduction </td>
              <td class="text-right strong-text">{{$PayrollSummery->deduction}} BDT</td>
            </tr>
            @if(isset($PayrollSummery->extends[0]))
    ...
    <tr>
      <td colspan="2" style="width: 50%">
        <table class="table">
            <tr>
              <th>Net Earning </th>
              <th class="text-right">{{$PayrollSummery->addition+$extendsAddition}} BDT</th>
            </tr>
        </table>
      </td>
      <td colspan="2" style="width: 50%">
        <table class="table">
            <tr>
              <th>Net Deduction </th>
              <th class="text-right">{{$PayrollSummery->deduction+$extendsDeduction}} BDT</th>
            </tr>
        </table>
      </td>
    </tr>
  ...
  <div class="row printbutton">
    <table class="table table-striped table-bordered">
      <caption><h4><center>Monthly Salary from date of execution - <strong>{{$PayrollSummery->date_of_execution}}</strong></center></h4></caption>
      <tr>
          ...
          <table class="table table-striped" style="border:0px">
            ...
            @foreach($payroll as $head)
              @if($head->head->head_type=="1")
                <tr>
                  <td>{{$head->head->head_name}}</td>
                  <td align="right">{{$head->amount}} BDT</td>
                </tr>
              @endif
            @endforeach
            @endif
